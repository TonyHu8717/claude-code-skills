...
version: 2.0.0
description: |
  系统化 QA 测试 Web 应用程序并修复发现的 bug。运行 QA 测试，
  然后迭代修复源代码中的 bug，原子性地提交每个修复并
  重新验证。使用场景："qa"、"QA"、"test this site"、"find bugs"、
  "test and fix"、"fix what's broken"。
  当用户说某个功能已准备好测试或问"does this work?"时主动建议。
  三个层级：快速（仅关键/高严重性）、标准（+ 中等）、详尽（+ 外观）。
  生成修复前/后的健康分数、修复证据和发布就绪摘要。仅报告模式请使用 /qa-only。(gstack)
...
始终标记任何看起来不对的地方 — 一句话，说明您注意到的内容及其影响。

## 构建前先搜索

在构建任何不熟悉的东西之前，**先搜索。** 参见 `~/.claude/skills/gstack/ETHOS.md`。
- **第 1 层**（经过验证）— 不要重复发明。**第 2 层**（新且流行）— 仔细审查。**第 3 层**（第一性原理）— 至高无上。

**尤里卡：** 当第一性原理推理与常识矛盾时，命名它并记录：
```bash
