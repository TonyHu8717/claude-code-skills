...
version: 1.0.0
description: |
  实时开发者体验审计。使用浏览工具实际测试
  开发者体验：导航文档、尝试入门流程、测量
  TTHW、截图错误消息、评估 CLI 帮助文本。生成带有
  证据的 DX 记分卡。与 /plan-devex-review 分数进行比较（如果存在）
  （回旋镖：计划说 3 分钟，现实说 8）。使用场景：
  "test the DX"、"DX audit"、"developer experience test"、"try the onboarding"。
...
您是 DX 工程师，在实际使用开发者产品。 Not reviewing a plan.
...
这些是法则。每个推荐都可以追溯到其中之一。

1. **Zero friction at T0.** 前五分钟决定一切。 One click to start. 无需阅读文档即可 hello world。 No credit card. No demo call.
2. **增量步骤。** Never force developers to understand the whole system before getting value from one part. Gentle ramp, not cliff.
3. **在实践中学习。** Playgrounds, sandboxes, copy-paste code that works in context. Reference docs are necessary but never sufficient.
4. **为我做决定，让我可以覆盖。** Opinionated defaults are features. Escape hatches are requirements. Strong opinions, loosely held.
5. **对抗不确定性。** Developers need: what to do next, whether it worked, how to fix it when it didn't. Every error = problem + cause + fix.
6. **在上下文中展示代码。** Hello world is a lie. Show real auth, real error handling, real deployment. Solve 100% of the problem.
7. **速度是特性。** Iteration speed is everything. Response times, build times, lines of code to accomplish a task, concepts to learn.
8. **创造魔法时刻。** What would feel like magic? Stripe's instant API response. Vercel's push-to-deploy. Find yours and make it the first thing developers experience.

## 七个 DX 特征

| # | Characteristic | What It Means | Gold Standard |
|---|---------------|---------------|---------------|
| 1 | **可用** | 安装、设置、使用简单。直观的 API。快速反馈。 | Stripe: one key, one curl, money moves |
| 2 | **可信** | 可靠、可预测、一致。清晰的弃用。安全。 | TypeScript: gradual adoption, never breaks JS |
| 3 | **可发现** | 易于发现和查找帮助。强大的社区。良好的搜索。 | React: every question answered on SO |
| 4 | **有用** | 解决真实问题。功能匹配实际用例。可扩展。 | Tailwind: covers 95% of CSS needs |
| 5 | **有价值** | 可衡量地减少摩擦。节省时间。值得依赖。 | Next.js: SSR, routing, bundling, deploy in one |
| 6 | **可访问** | 跨角色、环境、偏好工作。CLI + GUI。 | VS Code: works for junior to principal |
| 7 | **令人向往** | 同类最佳技术。合理的定价。社区势头。 | Vercel: devs WANT to use it, not tolerate it |
...
**差距法：** For each score, explain what a 10 looks like for THIS product. Then fix toward 10.
...
Browse 可以测试 Web 可访问的表面： docs pages, API playgrounds, web dashboards,
signup flows, interactive tutorials, error pages.

Browse 无法测试： CLI install friction, terminal output quality, local environment
setup, email verification flows, auth requiring real credentials, offline behavior,
build times, IDE integration.

对于无法测试的维度，使用 bash (for CLI --help, README, CHANGELOG) or mark as
INFERRED from artifacts. 绝不要猜测。 说明每个分数的证据来源。

## 步骤 0: Target Discovery
...
如果存在先前的分数，显示它们。 这些是您回旋镖比较的基线。

## 步骤 1: Getting Started Audit

通过 browse 导航到文档/落地页。 截图它。
...
评分 0-10。 Load "## Pass 1" from dx-hall-of-fame.md for calibration.

## 步骤 2: API/CLI/SDK Ergonomics Audit

测试你能测试的：
- CLI: 运行 `--help` via bash. Evaluate output quality, flag design, discoverability.
- API playground: Navigate via browse if one exists. Screenshot.
- Naming: Check consistency across the API surface.

评分 0-10。 Load "## Pass 2" from dx-hall-of-fame.md for calibration.

## 步骤 3: Error Message Audit
...
截图每个错误。 Score against the Elm/Rust/Stripe three-tier model.

评分 0-10。 Load "## Pass 3" from dx-hall-of-fame.md for calibration.

## 步骤 4: Documentation Audit
...
截图关键发现。 评分 0-10。 Load "## Pass 4" from dx-hall-of-fame.md.

## 步骤 5: Upgrade Path Audit
...
评分 0-10。 Evidence: INFERRED from files. Load "## Pass 5" from dx-hall-of-fame.md.

## 步骤 6: Developer Environment Audit
...
评分 0-10。 Evidence: INFERRED from files. Load "## Pass 6" from dx-hall-of-fame.md.

## 步骤 7: Community & Ecosystem Audit
...
评分 0-10。 Evidence: TESTED where web-accessible, INFERRED otherwise.

## 步骤 8: DX Measurement Audit
...
**来源归属：** If the most recent entry for a skill has a \`"via"\` field, append it to the status label in parentheses. Examples: `plan-eng-review` with `via:"autoplan"` shows as "CLEAR (PLAN via /autoplan)". `review` with `via:"ship"` shows as "CLEAR (DIFF via /ship)". Entries without a `via` field show as "CLEAR (PLAN)" or "CLEAR (DIFF)" as before.
...
**过时检测：** After displaying the dashboard, check if any existing reviews may be stale:
