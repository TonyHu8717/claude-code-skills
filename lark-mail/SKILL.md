...
| Shortcut | 说明 |
|----------|------|
| [`+message`](references/lark-mail-message.md) | 通过 message ID 读取单封邮件的完整内容。返回标准化的正文内容加附件元数据，包括内嵌图片。 |
| [`+messages`](references/lark-mail-messages.md) | 通过 message ID 批量读取多封邮件的完整内容。优先使用此 shortcut 而非直接调用原生 mail user_mailbox.messages batch_get，因为它会对 body 字段进行 base64url 解码并返回更易消费的标准化逐邮件输出。 |
| [`+thread`](references/lark-mail-thread.md) | 通过 thread ID 查询完整的邮件会话/线程。按时间顺序返回所有消息，包括回复和草稿，包含正文内容和附件元数据（含内嵌图片）。 |
| [`+triage`](references/lark-mail-triage.md) | 列出邮件摘要（日期/发件人/主题/message_id）。使用 --query 进行全文搜索，--filter 进行精确匹配条件过滤。 |
| [`+watch`](references/lark-mail-watch.md) | 通过 WebSocket 监听收信事件（需要 scope mail:event 和 bot 事件 mail.user_mailbox.event.message_received_v1）。运行时加 --print-output-schema 可在解析输出前查看各格式的字段参考。 |
| [`+reply`](references/lark-mail-reply.md) | 回复邮件并保存为草稿（默认）。使用 --confirm-send 在用户确认后立即发送。自动设置 Re: 主题、In-Reply-To 和 References 头部。 |
| [`+reply-all`](references/lark-mail-reply-all.md) | 回复所有收件人并保存为草稿（默认）。使用 --confirm-send 在用户确认后立即发送。自动包含所有原始收件人和抄送。 |
| [`+send`](references/lark-mail-send.md) | 撰写新邮件并保存为草稿（默认）。使用 --confirm-send 在用户确认后立即发送。 |
| [`+draft-create`](references/lark-mail-draft-create.md) | 从零创建全新邮件草稿（不用于回复或转发）。回复草稿使用 +reply；转发草稿使用 +forward。仅在撰写没有父邮件的新邮件时使用 +draft-create。 |
| [`+draft-edit`](references/lark-mail-draft-edit.md) | 在不发送的情况下更新现有邮件草稿。优先使用此 shortcut 而非直接调用原生 drafts.get 或 drafts.update，因为它执行草稿安全的 MIME 读取/修补/写入编辑，同时尽可能保留未更改的结构、附件和头部。 |
| [`+forward`](references/lark-mail-forward.md) | 转发邮件并保存为草稿（默认）。使用 --confirm-send 在用户确认后立即发送。自动包含原始邮件块。 |
| [`+send-receipt`](references/lark-mail-send-receipt.md) | 为请求了已读回执的收件邮件发送已读回执回复（即携带 READ_RECEIPT_REQUEST 标签的邮件）。正文自动生成（主题/收件人/发送时间/阅读时间）以匹配飞书客户端的回执格式 — 调用者无法自定义，符合已读回执正文为系统生成模板而非自由回复的行业惯例。供代理在用户确认后使用。 |
| [`+decline-receipt`](references/lark-mail-decline-receipt.md) | 通过清除收件邮件的 READ_RECEIPT_REQUEST 标签来消除已读回执请求提示，不发送回执。当用户希望静音提示但拒绝确认已阅读时使用。幂等 — 可安全重复运行。 |
| [`+signature`](references/lark-mail-signature.md) | 列出或查看邮件签名及默认使用信息。 |
| [`+share-to-chat`](references/lark-mail-share-to-chat.md) | 将邮件或会话以卡片形式分享到飞书 IM 群聊。 |
| [`+template-create`](references/lark-mail-template-create.md) | 创建个人邮件模板。扫描 HTML <img src> 本地路径（复用草稿内嵌图片检测），将内嵌图片和非内嵌附件上传到 Drive，将 HTML 重写为 cid: 引用，并将模板负载 POST 到 mail.user_mailbox.templates.create。 |
| [`+template-update`](references/lark-mail-template-update.md) | 更新现有邮件模板。支持 --inspect（只读投影）、--print-patch-template（打印 --patch-file 的 JSON 骨架）和扁平 flag（--set-subject / --set-name / 等）。内部 GET 模板、应用补丁、将 <img> 本地路径重写为 cid: 引用，然后 PUT 全量替换更新（无乐观锁：后写入者胜）。 |
