...
| Shortcut | 说明 |
|----------|------|
| [`+move`](references/lark-wiki-move.md) | 移动知识库节点，或将云空间文档移入知识库 |
| [`+node-create`](references/lark-wiki-node-create.md) | 创建知识库节点（自动解析空间） |
| [`+delete-space`](references/lark-wiki-delete-space.md) | 删除知识空间，需要时轮询异步删除任务 |
