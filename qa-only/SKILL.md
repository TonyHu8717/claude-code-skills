...
version: 1.0.0
description: |
  仅报告的 QA 测试。系统化测试 Web 应用程序并生成
  包含健康分数、截图和复现步骤的结构化报告——但绝不
  修复任何内容。使用场景："just report bugs"、"qa report only"、
  "test but don't fix"。完整的测试-修复-验证循环请使用 /qa。
  当用户想要不修改代码的 bug 报告时主动建议。(gstack)
